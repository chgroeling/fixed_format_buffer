...
TEST(SprintfComparison, Int_Zero) {
    FixedFormatBuffer<64> buf;
    buf.Format("%i", 0);
    EXPECT_EQ(buf.CStr(), Ref("%i", 0));
}

TEST(SprintfComparison, Int_Positive) {
    FixedFormatBuffer<64> buf;
    buf.Format("%i", 12345);
    EXPECT_EQ(buf.CStr(), Ref("%i", 12345));
}
...
TEST(SprintfComparison, Float_Zero) {
    FixedFormatBuffer<64> buf;
    buf.Format("%f", 0.0f);
    EXPECT_EQ(buf.CStr(), Ref("%f", 0.0f));
}

TEST(SprintfComparison, Float_One) {
    FixedFormatBuffer<64> buf;
    buf.Format("%f", 1.0f);
    EXPECT_EQ(buf.CStr(), Ref("%f", 1.0f));
}
...
TEST(SprintfComparison, Float_AlternateForm_Width_ZeroPad) {
    ...
    EXPECT_EQ(buf.CStr(), Ref("%#08.0f", 42.0f));
}
...
TEST(SprintfComparison, LengthMod_hd) {
    ...
    EXPECT_EQ(buf.CStr(), Ref("%hd", short(-1)));
}

TEST(SprintfComparison, LengthMod_hhd) {
    FixedFormatBuffer<64> buf;
    buf.Format("%hhd", static_cast<signed char>(-128));
    EXPECT_EQ(buf.CStr(), Ref("%hhd", static_cast<signed char>(-128)));
}
...
TEST(SprintfComparison, LengthMod_hu) {
    ...
    EXPECT_EQ(buf.CStr(), Ref("%hu", static_cast<unsigned short>(42)));
}
...
TEST(SprintfComparison, LengthMod_Lf) {
    ...
    EXPECT_EQ(buf.CStr(), Ref("%.1Lf", static_cast<long double>(3.1L)));
}
...
TEST(SprintfComparison, SpaceFlag_Int) {
    FixedFormatBuffer<64> buf;
    buf.Format("% d", 42);
    EXPECT_EQ(buf.CStr(), Ref("% d", 42));
}

TEST(SprintfComparison, SpaceFlag_Int_Negative) {
    FixedFormatBuffer<64> buf;
    buf.Format("% d", -7);
    EXPECT_EQ(buf.CStr(), Ref("% d", -7));
}

TEST(SprintfComparison, SpaceFlag_Float) {
    FixedFormatBuffer<64> buf;
    buf.Format("% .1f", 3.1f);
    EXPECT_EQ(buf.CStr(), Ref("% .1f", 3.1f));
}
...
TEST(SprintfComparison, StarPrecision_Float) {
    ...
    EXPECT_EQ(buf.CStr(), Ref("%.*f", 2, 3.14159f));
}
